-- Fix the set_notification_time function - add WHERE clause to UPDATE
...
BEGIN
  ...
  -- Upsert settings (single row) - FIXED: Added WHERE clause
  IF EXISTS (SELECT 1 FROM public.notification_settings LIMIT 1) THEN
    ...
    WHERE TRUE; -- Added WHERE clause to fix the error
  ELSE
