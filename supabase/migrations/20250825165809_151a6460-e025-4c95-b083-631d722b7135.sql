-- Make set_notification_time robust to missing jobs
...
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'send-notifications-1935-europe-istanbul') THEN
  ...
  -- Schedule the daily job (name is stable)
  PERFORM cron.schedule(
    'send-notifications-daily',
    cron_expr,
    'SELECT extensions.net.http_post(url:=''https://outngvycajxdgyellntw.supabase.co/functions/v1/send-notifications'', headers:=''{\"Content-Type\": \"application/json\"}''::jsonb);'
  );
END;
$$;